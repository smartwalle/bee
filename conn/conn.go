...
func (w *messageWriter) flushFrame(final bool, extra []byte) error {
	...
	if !c.isServer {
		...
		if len(extra) > 0 {
			return w.endMessage(c.writeFatal(errors.New("socket: internal error, extra used in client mode")))
		}
	}
	...
	if c.isWriting {
		panic("concurrent write to socket connection")
	}
	c.isWriting = true
...
func (c *Conn) handleProtocolError(message string) error {
	...
	return errors.New("socket: " + message)
}
...
func (c *Conn) NextReader() (messageType int, r io.Reader, err error) {
	...
	if c.readErrCount >= 1000 {
		panic("repeated read on failed socket connection")
	}
...
func (r *messageReader) Read(b []byte) (int, error) {
	...
	for c.readErr == nil {
		...
		case frameType == TextMessage || frameType == BinaryMessage:
			c.readErr = errors.New("socket: internal error, unexpected text or binary in Reader")
		}
	}
